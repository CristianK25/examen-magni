            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="border border-gray-300 bg-white rounded shadow-sm h-fit">
                    <div className="bg-gray-100 p-3 border-b border-gray-300 font-bold text-sm text-gray-800 rounded-t flex justify-between items-center">
                        ...
                        <p className="text-green-700">Presupuesto: {formatearDinero(montoDisponible)}</p>
                    </div>
                                    ...
                                    productosDisponibles.map((producto: Producto) => (
                                        <tr key={producto.codigoProducto} className="hover:bg-gray-50">
                                            ...
                                            <td className="border border-gray-300 p-2 text-center">
                                                <button
                                                    onClick={() => agregarProducto(producto.codigoProducto)}
                        ...
                        <table className="w-full border-collapse border border-gray-300 text-left text-xs">
                            <thead>
                                <tr>
                                    ...
                                    <th className="sticky top-0 bg-gray-200 z-10 border border-gray-300 p-2 font-bold text-gray-700">Categoría</th>
                                    <th className="sticky top-0 bg-gray-200 z-10 border border-gray-300 p-2 font-bold text-gray-700">Precio</th>
                                    <th className="sticky top-0 bg-gray-200 z-10 border border-gray-300 p-2 font-bold text-gray-700 text-center">Cant</th>
                                ...
                                {productosSeleccionados.length === 0 ? (
                                    ...
                                    productosSeleccionados.map((producto: Producto) => (
                                        <tr key={producto.codigoProducto} className="hover:bg-gray-50">
                                            ...
                                            <td className="border border-gray-300 p-2 text-gray-600">{producto.denominacion}</td>
                                            <td className="border border-gray-300 p-2 text-gray-600">{producto.categoria}</td>
                                            <td className="border border-gray-300 p-2 text-gray-900">{formatearDinero(producto.precioVenta)}</td>
                                            <td className="border border-gray-300 p-2 text-center font-bold">{producto.cantidad}</td>
                                            <td className="border border-gray-300 p-2 text-gray-900 font-semibold">{formatearDinero(producto.precioVenta * (producto.cantidad || 1))}</td>
                                            <td className="border border-gray-300 p-2 text-center">
                                                <button
